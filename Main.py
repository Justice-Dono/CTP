...
#This is an array of all combat actions.
global COMBAT_POSITIONS
COMBAT_POSITIONS = None

#This keeps track of the return from combat. I'm not sure it needs to be global, but this is my project, not a good project.
...
#This function moves the pointer turtle up and down in the array.
...
def move_up():
    global global_index
    if global_index == 0:
		#If the index will be exceeded, we set the index to the maximum value to create a loop effect.
        global_index = 3
    else:
		#Otherwise, we move it up one index.
        global_index -= 1
...
def move_down():
    global global_index
    if global_index == 3:
		#If the index will be exceeded, we set the index to the minimum value to create a loop effect.
        global_index = 0
    else:
		#Otherwise, we move it down one index.
        global_index += 1
    move(global_cursor, global_index, COMBAT_POSITIONS)
...
def enter():
	global combat_return
	#We set the return to a, d, i, or r to get the command.
	if(global_index == 0):
		combat_return = "a"
	if(global_index == 1):
		combat_return = "d"
